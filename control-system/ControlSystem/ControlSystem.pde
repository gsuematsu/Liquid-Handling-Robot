...
void setup() {
    ...
    mX = new Motor(port, "X", 12, 13);
    mY = new Motor(port, "Y", 10, 11);
    mZ = new Motor(port, "Z", 6, 7);
    robot = new LiquidHandlingRobot(port, mX, mY, mZ, tsa, 300, 300, 10);
}

void draw() {

    if (port.available() > 0) val = port.readStringUntil('\n');
